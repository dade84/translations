...
<!-- to curate (курировать-вести, направл€ть, опекать) 
...
<!-- Frankly (пр€мо; открыть; откровенно; откровенно говор€; пр€мо скажем; если честно;)
to denote (обозначать; означать;)
outline (иерархическа€ структура документа(вчт.))
flavour (вид)
consider (рассмотреть вопрос)
appropriate ( соответствующий; подход€щий;)
...
<!-- the rest of (остальной; оставшийс€; остальна€ часть чего-либо;) -->
...
## Overuse of header ##
<!-- overuse (злоупотребл€ть; чрезмерно использовать;) -->
...
popularize (распростран€ть; пропагандировать; излагать попул€рно; попул€ризировать;)
...
to ensure (гарантировать; обеспечивать; ручатьс€;)
...
abuse (чрезмерно увлекатьс€;)
intend (преполагать;предназначать;)
terms of service (услови€ оказани€ услуг;)
alone (как таковой; вз€тый отдельно; вз€тый сам по себе;)
...
coupled with (в сочетании с; вкупе с; нар€ду с;)
...
<!-- 
unsure (неуверенный; не гарантированный; небезопасный; колеблющийс€;)
consider (рассматривать; считать;)
rules of thumb (практические методы; правила на основе практики;)
appropriate (соответствующий; оптимальный; подход€щий;)
-->
...
<!--
partner-in-crime (сообщник;подельник; соучастник преступлени€;)
along with (вместе; нар€ду с; вместе с;)
-->
...
optionally (необ€зательно; при желании; в качестве опции; добавочный; дополнительный;)
caption (сопроводительна€ подпись; заголовок главы; заглавие; подпись к картинке;)
self-contained (самосто€тельный; независимый; замкнутый; автономный;)
typically (типично; как правило; часто; в основном; обычно;)
single unit (одну единица)
therein (в этом; в этом отношении;)
therein lies (в этом заключаетс€;)
moved away (отодвинутый;)
sidebar (бокова€ панель;)
affect (затрагивать;)
purely (чисто; совершенно; полностью; только; лишь; просто;)
vary (мен€ть; мен€тьс€; измен€тьс€; отличатьс€;расходитьс€; варьировать;)
appendix (приложение;)
-->
...
ThereТs nothing else to say here. ItТs just plain wrong. We could argue until the cows come home about whether your logo should be in an <h1>, but thatТs not what weТre here for. The real issue is the abuse of the <figure> element. <figure> should be used only when referenced in a document or surrounding sectioning element. I think itТs fair to say that your logo is rarely going to be referenced in such a way. Quite simply, donТt do it. All you need is this:
...
<!--
seguing (продолжа€)
snippet (фрагмент кода; фрагмент; коротка€ текстова€ информаци€ по сайту;)
plain (€вный; простой;€сный; плоский;однотонный;)
argue (спорить;приводить доводы; доказывать; рассуждать; высказыватьс€;)
...
Another common misconception regarding <figure> is that it can only be used for images. This isnТt the case. A <figure> could be video, audio, a chart (in SVG, for example), a quote, a table, a block of code, a piece of prose, or any combination of these and much more besides. DonТt limit your <figure>s to images. Our job as web standards affectionadoes is to accurately describe the content with our markup.

A while back, I wrote about <figure> in more depth. ItТs worth a read if you want more detail or need a refresher.
...
besides ()
affectionadoes ()
a while back (некоторое врем€ тому назад)
it is worth (имеет смысл)
refresher (повторный)

-->

## DonТt include unnecessary type attributes ##

This is probably the most common problem we see in HTML5 Gallery submissions. While it isnТt really a mistake, I believe itТs best practice to avoid this pattern.

In HTML5, thereТs no need to include the type attribute on <script> and <style> elements. While these can be a pain to remove if theyТre automatically added by your CMS, thereТs really no reason to include them if youТre coding by hand or if you have tight control over your templates. Since all browsers expect scripts to be JavaScript and styles to be CSS, you donТt need this:

	<!-- DonТt copy this code! ItТs attribute overload! -->
...
You can also reduce the amount of code you write to specify your character set, amongst other things. Mark PilgrimТs chapter on semantics in Dive into HTML5 explains all.
...
You may be aware that HTML5 has introduced a range of new attributes for forms. WeТll cover them in more detail in the coming months, but in the meantime, IТll quickly show you a few things not to do.

<!--
range (р€д)
in the meantime (в то же врем€; между тем; а пока; тем временем;)
-->
...
Admittedly, I only rarely see this, but using required as an example, IТve seen the following:

	<!-- DonТt copy this code! ItТs wrong! -->
	<input type="email" name="email" required="true" />
	<!-- Another bad example -->
	<input type="email" name="email" required="1" />

Ultimately, this causes no harm. The clientТs HTML parser sees the required attribute in the markup, so its function will still be applied. But what if you flip the code on its head and type required="false"?
...
There are three valid ways for a boolean attribute to be applied. (The second and third options only apply if youТre writing XHTML syntax.)
...
It would be impossible for me to list here all the quirky markup patterns and practices IТve come across, but these are some of the most frequently seen. What other common markup mistakes have you spotted around the web? Which areas of HTML5 require further clarification? WeТd love to help get the wording or examples in the spec changed to make them a little more specific, so leave your thoughts below, and donТt forget to escape your HTML!
...
<!--
quirky (изворотливый)
come across (натолкнутьс€; повстречатьс€; случайно встретитьс€;)
to spot (обнаружить;заметить;)
clarification (про€снение; вы€снение; разъ€снение;)
to escape (спасатьс€;)
-->
